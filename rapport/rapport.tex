...
Au vu de cette nouvelle il faut désormais refaire des tests. Mais on se restreindra cette fois au heuristique d'amélioration les plus prometeuses :
\begin{itemize}
\item
...
\item
vnd comme solution intermédiaire qui pourrait se révélé intéressante
\end{itemize}

Notre algorithme GRASP commence toujours par construire une solution avec un NNH déterministe avant de l’amélioré avec VND. Le but étant d'avoir une bonne solution dès le début. Mais lorsque GRASP utilise l’opérateur 2-opt on se rend compte qu'une part importante de son temps d’exécution est due à cette première recherche. En effet vnd avait été choisis pour cette tache car il produit de bonnes solutions mais il n'est pas trop lent (surtout qu'il n'est utilisé qu'une seule fois). Mais étant donné la vitesse de GRASP avec 2-opt on va remplacer, dans les exécutions avec -opt, cette amélioration par une descente avec 2-opt.
